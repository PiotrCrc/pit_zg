...
class PositionSummary:
                ...
                if (transaction_record.operation == "TRADE"):
                    if (transaction_record.currency == self.symbol):
                        self.transactions.append(
                            self.Transaction(transaction_record.date_time, transaction_record.amount, 0, 0, 0))
                    else:
                        self.transactions.append(
                            self.Transaction(transaction_record.date_time, 0, transaction_record.amount, 0,
                                             transaction_record.currency_rate_day_before))
                elif (transaction_record.operation == "COMMISSION"):
                    self.transactions.append(self.Transaction(transaction.date_time, 0, 0, transaction_record.amount,
                                                              transaction_record.currency_rate_day_before))
                elif (transaction_record.operation == "TAX"):
                    self.dividends.append(
                        self.Transaction(transaction_record.date_time, 0, 0, transaction_record.amount,
                                         transaction_record.currency_rate_day_before))
                elif (transaction_record.operation == "DIVIDEND"):
                    self.dividends.append(
                        self.Transaction(transaction_record.date_time, transaction_record.amount, 0, 0,
                                         transaction_record.currency_rate_day_before))
        ...
        for transaction in self.transactions:
            if (transaction.amount > 0):
                self.actual_positions.append(
                    self.Position(transaction.date_time, transaction.amount, transaction.price, transaction.rate,
                                  transaction.fee))
                ...
                if (transaction.fee < 0):
                    ...
                    log.append("Koszt przy sprzedarzy " + self.symbol + " dnia " + str(transaction.date_time.date()) + \
                               " o kwocie " + str(transaction.fee) + " " + self.currency + " po kursie " + str(
                        transaction.rate) + \
                ...
                for position in self.actual_positions:
                    if (position.amount >= temp_amount):
                        ...
                        log.append("Sprzedarz " + str(temp_amount) + " " + self.symbol + \
                                   ...
                                   " o wartości " + str(
                            transaction.price / transaction.amount * temp_amount) + " " + self.currency + " " \
                                                                                                          " po kursie " + str(
                            transaction.rate) + \
                                   " kupionych " + str(position.date_time.date()) + \
                                   " po cenie " + str(
                            round(position.price / position.amount, 4)) + " za akcję po kursie " + str(position.rate) + \
                                   " co daje " + str(
                            round(((transaction.price / transaction.amount) * transaction.rate * (-1.0) \
                                   - (position.price / position.amount) * position.rate * (-1.0)) \
                        ...
                        if (position.amount > temp_amount):
                            ...
                            self.tax.append((str(transaction.date_time), \
                                             ...
                                             "koszt przy zakupie"))
                            log.append(
                                "Koszt przy zakupie " + self.symbol + " dnia " + str(transaction.date_time.date()) + \
                                " o kwocie " + str(position.fee * ratio) + " " + self.currency + " po kursie " + str(
                                    position.rate) + \
                            ...
                            log.append(
                                "Koszt przy zakupie " + self.symbol + " dnia " + str(transaction.date_time.date()) + \
                                " o kwocie " + str(position.fee) + " " + self.currency + " po kursie " + str(
                                    position.rate) + \
                        ...
                        log.append("Sprzedarz " + str(position.amount) + " " + self.symbol + \
                                   " dnia " + str(transaction.date_time.date()) + \
                                   " o wartości " + str(
                            position.amount / transaction.amount * transaction.price) + " " + self.currency + " " \
                                                                                                              " po kursie " + str(
                            transaction.rate) + \
                                   " kupionych " + str(position.date_time.date()) + \
    ...
    class Transaction:
        ...
        def __repr__(self):
            return self.date_time.strftime("%y-%m-%d %H:%M:%S \t") + str(self.amount) + "\t" + str(
                self.price) + "\t" + str(self.fee) + "\t" + str(self.rate)
...
if __name__ == "__main__":
    ...
    with open('data/ex.csv', encoding='utf16') as csv_file:
        rows = csv.reader(csv_file, delimiter='\t')
        for row in rows:
            if (row[0].isdigit()):
                transactions.insert(0, TransactionRecord(int(row[0]), row[2], row[3], row[4], float(row[5]), row[6],
                                                         float(row[7]), row[1], row[8]))
                if (len(row[6]) == 3):
                    if [row[6], 0] not in list_of_currencies:
                        exchange_rates.append(ExchangeRate(row[6]))
                        list_of_currencies.append([row[6], 0])

    with open('data/rates2020.csv') as csv_file:
        rows = csv.reader(csv_file, delimiter=';')
        first_row = 1
        for row in rows:
            if (first_row == 1):
                ...
                if (row[0].isdigit()):
                    for i in range(len(exchange_rates)):
                        exchange_rates[i].add_rate(datetime.datetime.strptime(row[0], '%Y%m%d').date(),
                                                   float(row[list_of_currencies[i][1]].replace(",", ".")))

    for transaction in transactions:
        if (len(transaction.currency) == 3):
            for exchange_rate in exchange_rates:
                if (exchange_rate.symbol == transaction.currency):
                    transaction.currency_rate_day_before = exchange_rate.get_previous_date_rate(
    ...
    with open('data/ex_out.csv', 'w') as csv_file:
